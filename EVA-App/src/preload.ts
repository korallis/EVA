...
contextBridge.exposeInMainWorld('electronAPI', {
  ...
  esi: {
    ...
    getCharacterMarketOrders: (characterId?: string) => 
      ipcRenderer.invoke('esi:getCharacterMarketOrders', characterId),
    getCharacterMarketOrderHistory: (characterId?: string, page?: number) => 
      ipcRenderer.invoke('esi:getCharacterMarketOrderHistory', characterId, page),
    getRegionalMarketOrders: (regionId: number, typeId?: number, page?: number) => 
      ipcRenderer.invoke('esi:getRegionalMarketOrders', regionId, typeId, page),
    getMarketPrices: () => 
      ipcRenderer.invoke('esi:getMarketPrices'),
    getMarketHistory: (regionId: number, typeId: number) => 
      ipcRenderer.invoke('esi:getMarketHistory', regionId, typeId),
    
    // Wallet Endpoints (High Priority)
    getCharacterWalletJournal: (characterId?: string, page?: number) => 
      ipcRenderer.invoke('esi:getCharacterWalletJournal', characterId, page),
    getCharacterWalletTransactions: (characterId?: string) => 
      ipcRenderer.invoke('esi:getCharacterWalletTransactions', characterId),
    
    // Industry Endpoints (High Priority)
    getCharacterIndustryJobs: (characterId?: string, includeCompleted?: boolean) => 
      ipcRenderer.invoke('esi:getCharacterIndustryJobs', characterId, includeCompleted),
    getCharacterMiningLedger: (characterId?: string, page?: number) => 
      ipcRenderer.invoke('esi:getCharacterMiningLedger', characterId, page),
    
    // Contracts Endpoints (Medium Priority)
    getCharacterContracts: (characterId?: string, page?: number) => 
      ipcRenderer.invoke('esi:getCharacterContracts', characterId, page),
    getContractItems: (characterId: string, contractId: number) => 
      ipcRenderer.invoke('esi:getContractItems', characterId, contractId),
    
    // Mail Endpoints (Medium Priority)
    getCharacterMail: (characterId?: string, lastMailId?: number) => 
      ipcRenderer.invoke('esi:getCharacterMail', characterId, lastMailId),
    
    // Contacts Endpoints (Medium Priority)
    getCharacterContacts: (characterId?: string, page?: number) => 
      ipcRenderer.invoke('esi:getCharacterContacts', characterId, page),
    
    // Killmails Endpoints (Medium Priority)
    getCharacterKillmails: (characterId?: string, page?: number) => 
      ipcRenderer.invoke('esi:getCharacterKillmails', characterId, page),
    getKillmailDetails: (killmailId: number, hash: string) => 
      ipcRenderer.invoke('esi:getKillmailDetails', killmailId, hash),
    
    // Enhanced Assets Endpoints
    getCharacterAssetLocations: (characterId: string, itemIds: number[]) => 
      ipcRenderer.invoke('esi:getCharacterAssetLocations', characterId, itemIds),
    getCharacterAssetNames: (characterId: string, itemIds: number[]) => 
      ipcRenderer.invoke('esi:getCharacterAssetNames', characterId, itemIds),
    
    // Comprehensive Data Refresh
    refreshEnhancedCharacterData: (characterId?: string) => 
      ipcRenderer.invoke('esi:refreshEnhancedCharacterData', characterId)
  },
...
export interface ElectronAPI {
  ...
  esi: {
    ...
    // Market Endpoints (High Priority)
    getCharacterMarketOrders: (characterId?: string) => Promise<any[]>;
    getCharacterMarketOrderHistory: (characterId?: string, page?: number) => Promise<any[]>;
    getRegionalMarketOrders: (regionId: number, typeId?: number, page?: number) => Promise<any[]>;
    getMarketPrices: () => Promise<any[]>;
    getMarketHistory: (regionId: number, typeId: number) => Promise<any[]>;
    
    // Wallet Endpoints (High Priority)
    getCharacterWalletJournal: (characterId?: string, page?: number) => Promise<any[]>;
    getCharacterWalletTransactions: (characterId?: string) => Promise<any[]>;
    
    // Industry Endpoints (High Priority)
    getCharacterIndustryJobs: (characterId?: string, includeCompleted?: boolean) => Promise<any[]>;
    getCharacterMiningLedger: (characterId?: string, page?: number) => Promise<any[]>;
    
    // Contracts Endpoints (Medium Priority)
    getCharacterContracts: (characterId?: string, page?: number) => Promise<any[]>;
    getContractItems: (characterId: string, contractId: number) => Promise<any[]>;
    
    // Mail Endpoints (Medium Priority)
    getCharacterMail: (characterId?: string, lastMailId?: number) => Promise<any[]>;
    
    // Contacts Endpoints (Medium Priority)
    getCharacterContacts: (characterId?: string, page?: number) => Promise<any[]>;
    
    // Killmails Endpoints (Medium Priority)
    getCharacterKillmails: (characterId?: string, page?: number) => Promise<any[]>;
    getKillmailDetails: (killmailId: number, hash: string) => Promise<any | null>;
    
    // Enhanced Assets Endpoints
    getCharacterAssetLocations: (characterId: string, itemIds: number[]) => Promise<any[]>;
    getCharacterAssetNames: (characterId: string, itemIds: number[]) => Promise<any[]>;
    
    // Comprehensive Data Refresh
    refreshEnhancedCharacterData: (characterId?: string) => Promise<any>;
  };
  sde: {

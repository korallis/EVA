...
import { authLogger, EVALogger } from '../../utils/logger';
...
export class AuthService {
  ...
  private async exchangeCodeForToken(code: string): Promise<AuthData | null> {
    try {
      ...
      EVALogger.success('auth', 'Token exchange successful', { 
        characterId: parseInt(tokenData.sub?.split(':')[2] || '0'),
      ...
      throw ErrorHandler.createError(
        ...
        ErrorSeverity.HIGH,
        {
          ...
          userFriendly: 'Authentication failed. Please try logging in again.'
        }
      );
    }
  }

  // Store auth data securely using encrypted storage
  private async storeAuthData(authData: AuthData): Promise<void> {
    try {
      // Use secure encrypted storage
      await secureTokenStorage.storeAuthData(authData);
      
      // Check for and migrate old plaintext storage
      const userDataPath = app.getPath('userData');
      const oldAuthFilePath = path.join(userDataPath, 'auth.json');
      
      if (fs.existsSync(oldAuthFilePath)) {
        console.log('🔄 Removing old plaintext auth file for security');
  ...
  private async loadAuthData(): Promise<AuthData | null> {
    try {
      ...
      let authData = await secureTokenStorage.loadAuthData();
      
      // If no secure data, try to migrate from old plaintext storage
      if (!authData) {
        const userDataPath = app.getPath('userData');
        const oldAuthFilePath = path.join(userDataPath, 'auth.json');
        
        const migrationSuccess = await secureTokenStorage.migrateFromPlaintextStorage(oldAuthFilePath);
        
        if (migrationSuccess) {
          authData = await secureTokenStorage.loadAuthData();
        }
      }
      ...
      // Check if token is expired (secure storage already does this, but double-check)
  ...
  async logout(): Promise<void> {
    try {
      ...
      // Also clean up any old plaintext files for security
      const userDataPath = app.getPath('userData');
      const oldAuthFilePath = path.join(userDataPath, 'auth.json');
      
      if (fs.existsSync(oldAuthFilePath)) {
        await fs.promises.unlink(oldAuthFilePath);
        console.log('🧹 Removed old plaintext auth file during logout');
      }
  ...
  handleUrlSchemeCallback(url: string): void {
    if (url.startsWith('eva://') || url.startsWith('evaapp://')) {
      console.log('📞 Received URL callback:', url);
      // The callback handling is already implemented in the existing auth flow
      // This method serves as a bridge for the app-level URL scheme handling
    }
  }
}

...
import { cacheLogger, EVALogger } from '../../utils/logger';
...
export class CacheService {
  ...
  // Store for tracking in-flight requests to prevent duplicate API calls
  private inflightRequests = new Map<string, Promise<any>>();
  
  // Background refresh queue for proactive cache updates
  ...
  constructor() {
    ...
    EVALogger.success('cache', 'Cache service initialized');
  }
  ...
  async cached<T>(
    ...
    if (this.inflightRequests.has(key)) {
      cacheLogger.debug(`Request already in flight, waiting: ${key}`);
  ...
  // Fetch with automatic retry logic for better reliability
    ...
    for (let attempt = 0; attempt < maxRetries; attempt++) {
      try {
        ...
        if (attempt < maxRetries - 1) {
          ...
          cacheLogger.warning(`Fetch retry ${attempt + 1}/${maxRetries} in ${delay}ms: ${key}`);
          await new Promise(resolve => setTimeout(resolve, delay));
        }
      }
    }
    
    cacheLogger.error(`All fetch attempts failed: ${key}`, lastError);
    throw lastError;
  }

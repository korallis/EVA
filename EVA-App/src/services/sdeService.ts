...
import { sdeCache, CACHE_CONFIGS } from './cacheService';
import { sdeLogger, EVALogger } from '../utils/logger';
...
class SDEService {
  ...
  async initialize(): Promise<void> {
    ...
    EVALogger.info('sde', `Initializing SDE Service at ${this.dbPath}`);
    ...
    return new Promise((resolve, reject) => {
      this.db = new Database.Database(this.dbPath, (err) => {
        if (err) {
          const error = ErrorHandler.createError(
            ...
            ErrorSeverity.CRITICAL,
            {
              service: 'sde',
              cause: err,
              context: `Database path: ${this.dbPath}`,
              userFriendly: 'Unable to access game database. Please restart the application.'
            }
          );
          reject(error);
          return;
        ...
        EVALogger.success('sde', 'SDE database connected successfully');
        this.createTables()
          .then(() => {
            this.isInitialized = true;
            EVALogger.success('sde', 'SDE Service initialization complete');
            resolve();
          })
          .catch((error) => {
            const wrappedError = ErrorHandler.createError(
              ...
              ErrorSeverity.CRITICAL,
              {
                service: 'sde',
                cause: error,
                userFriendly: 'Database initialization failed. Please check application permissions.'
              }
            );
            reject(wrappedError);
          });
  ...
  async getShips(): Promise<ShipType[]> {
    return sdeCache.getOrSet('ships', async () => {
    ...
    }, CACHE_CONFIGS.SDE_SHIPS.ttl);
  }

  async getModules(): Promise<ModuleType[]> {
    return sdeCache.getOrSet('modules', async () => {
    ...
    }, CACHE_CONFIGS.SDE_MODULES.ttl);
  }

  async getTypeAttributes(typeID: number): Promise<TypeAttribute[]> {
    return sdeCache.getOrSet(`attributes:${typeID}`, async () => {
      ...
      `, [typeID]);
    }, CACHE_CONFIGS.SDE_ATTRIBUTES.ttl);
  }

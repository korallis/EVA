...
import { startupLogger, EVALogger } from '../utils/logger';
...
export class StartupSDEManager {
  ...
  private reportProgress(
    ...
    EVALogger.startup('startup', `SDE [${stage}] ${progress.toFixed(1)}% - ${message}`);
  }
  ...
  async checkAndUpdateSDE(): Promise<boolean> {
    try {
      ...
      if (!installedVersion) {
        EVALogger.info('startup', 'No SDE data found - first run setup required');
      ...
      const wrappedError = ErrorHandler.createError(
        'Startup SDE check failed',
        ErrorCategory.STARTUP,
        ErrorSeverity.HIGH,
        {
          service: 'startup',
          cause: error as Error,
          userFriendly: 'Failed to check SDE data. The application may work with existing data.'
        }
      );
      
      EVALogger.error('startup', 'Startup SDE check failed', wrappedError);
      this.reportProgress('error', 0, `SDE check failed: ${error.message}`, false, true, error.message);
      
      // Try to use existing data if available
      const installedVersion = await sdeDownloader.getInstalledVersion();
      if (installedVersion) {
        EVALogger.warning('startup', 'Using existing SDE data despite check failure');
        return await this.validateExistingSDE();
      }
